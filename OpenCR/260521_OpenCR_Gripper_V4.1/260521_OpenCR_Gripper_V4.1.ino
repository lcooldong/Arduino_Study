...
#define GRIP_MOVE_TIMEOUT_MS    2000
#define PUSH_MOVE_TIMEOUT_MS    4000
...
const unsigned long CAN_RX_TIMEOUT_MS = 150;
...
void loop() {
   ...
   // 4. 통신선 탈락 예외 처리 (3초 동안 마스터 패킷이 안 오면 송신을 잠가서 하드웨어 보호)
    if (can_bus_active && !isCanRecentlyAlive()) {
        ...
        need_send_heartbeat = false; // 20260521 추가
            ...
            switch (rxPkt->cmd)
            {
                case CMD_MOVE:  // 0x02
                {
                    ...
                    if(targetId == GRIPPER_ID) {
                        ...
                        pusherPrevTarget = dxl.getPresentPosition(PUSHER_ID);
...
void sendHeartbeatPacket()
{
    ...
    if (pending_error) {
        txPkt.cmd = CMD_ERROR;
        txPkt.id = last_error_motor_id;
        txPkt.data.status_res.error_code = system_Status.data.status_res.error_code;
    } else {
        txPkt.cmd = CMD_HEARTBEAT;
        txPkt.id = 0x00;
        txPkt.data.status_res.error_code = ERROR_NONE;
    }
...
void checkArrival() {
    ...
    if (isGripperMoving) {
        ...
        if (abs((long)presentPos - (long)gripper_Target) <= ARRIVE_THRESHOLD) {
            ...
            if (isGripperReturning) {
                isGripperReturning = false;
                DEBUG_SERIAL.println("GRIPPER Return Completed");
            }
    ...
    if (isPusherMoving) {
        ...
        if (abs((long)presentPos - (long)pusher_Target) <= ARRIVE_THRESHOLD) {
            ...
            if (isPusherReturning) {
                isPusherReturning = false;
                DEBUG_SERIAL.println("PUSHER Return Completed");
            }
...
void sendArrivedPacket(uint8_t id, uint16_t pos) {
    ...
    // 최신 정보 채우기
    txPkt.data.status_res.gripper_pos = 
        (id == GRIPPER_ID) ? pos : (uint16_t)dxl.getPresentPosition(GRIPPER_ID);
    txPkt.data.status_res.pusher_pos = 
        (id == PUSHER_ID) ? pos : (uint16_t)dxl.getPresentPosition(PUSHER_ID);
...
void checkMoveTimeout() {
    ...
    if (!isGripperReturning) {
        ...
        isGripperReturning = true;
        gripper_Target = DXL_OPEN_POSITION;
        gripperMoveStartMillis = currMillis;
        dxl.setGoalPosition(GRIPPER_ID, gripper_Target);
    } else {
        setMotorError(GRIPPER_ID, ERROR_RETURN_TIMEOUT);
    ...
    if (!isPusherReturning) {
        ...
        isPusherReturning = true;
        pusher_Target = DXL_RELEASE_POSITION;
        pusherMoveStartMillis = currMillis;
        dxl.setGoalPosition(PUSHER_ID, pusher_Target);
    } else {
        setMotorError(PUSHER_ID, ERROR_RETURN_TIMEOUT);
